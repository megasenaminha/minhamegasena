...
async function planilhaDb() {
  ...
  // Função para extrair as dezenas de 1 a 6 de um objeto
  function extrairDezenas(planilhaData) {
    var dezenas = [];
    for (var i = 1; i <= 6; i++) {
      dezenas.push(planilhaData["dezena" + i]);
    }
    return dezenas;
  }

  // Processando cada objeto na matriz
  var todasDezenas = planilhaData.map(extrairDezenas);

  // Imprimindo todas as dezenas
  //console.log(todasDezenas);
  return todasDezenas;
}
...
async function obtemGrupoFrequencia() {
  const dezenas = await planilhaDb();
